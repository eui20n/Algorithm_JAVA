...
package baekjoon.t17000f17999.p17070;
...
public class Main {
    ...
    static void installPipe() {
        int[][][] pipe = new int[3][N + 1][N + 1]; // 가로(0), 세로(1), 대각선(2)
        ...
        int result = pipe[0][N][N] + pipe[1][N][N] + pipe[2][N][N];
        System.out.println(result);
    }

    static void horizontal(int[][][] pipe, int x, int y) {
        // 가로
        if (y <= N) {
    ...
    static void vertical(int[][][] pipe, int x, int y) {
        // 세로
        if (x <= N) {
    ...
    static void diagonal(int[][][] pipe, int x, int y) {
        // 대각선
        if (x <= N && y <= N) {
            // 벽인 위치에서는 올 수 없음
...
// 가로, 세로, 대각선 3가지 경우로 나눠서 계산할 것
// 각 자리의 값은 각 자리에서 갈 수 있는 경우의 수
